...
from rpi_arduino_com import serial_RPiArduino, arduinoUSB
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket_com:
            ...
            while True:
                ...
                while True:
                    data = client_socket_com.recv(1024).decode().upper()
                    data = data.strip()
                    ...
                    if data == 'END':
                        ...
                        client_socket_com.send("1".encode())
                        client_socket_com.send(f"Command '{data}' received!".encode())
                        serial_RPiArduino(stringReceived="1")
        ...
        print("Closing all communications")
    finally:
        if arduinoUSB is not None:
            serial_RPiArduino(stringReceived="STOP")
            time.sleep(0.1)
            serial_RPiArduino(stringReceived="c")
            time.sleep(0.1)
            serial_RPiArduino(stringReceived="0")
            time.sleep(0.1)
            serial_RPiArduino(close=1)
        if server_socket_com is not None:
            server_socket_com.close()
        if client_socket_com is not None:
            client_socket_com.close()
